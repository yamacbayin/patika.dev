...
public class Mines extends BattleZone {
    ...
    public Mines(String name, String description, List<Enemy> enemyList, Random random) {
        super(name, description, enemyList, random);
    }

    public Mines(String name, String description, Random random) {
    ...
    public void interact(Scanner scanner, Player player, PlayerMovementService playerMovementService, GameMap gameMap) {
        ...
        List<Item> drops = new ItemFactory(random).createDrops(2, 8,
                30, 80,
                60, 80,
                50, 80);
            ...
            switch (selection) {
                case 0 -> {
                    if (alreadyExplored) {
                        ...
                        if (battleResult) {
                            EndGameUtil.setIsElementalGemstoneCollected(Boolean.TRUE);
