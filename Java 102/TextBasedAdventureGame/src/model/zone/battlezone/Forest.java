...
public class Forest extends BattleZone {
    ...
    public Forest(String name, String description, Random random) {
        super(name, description, random);
    }

    public Forest(String name, String description, List<Enemy> enemyList, Random random) {
    ...
    public void interact(Scanner scanner, Player player, PlayerMovementService playerMovementService, GameMap gameMap) {
        ...
        List<Item> drops = new ItemFactory(random).createDrops(2, 4,
                10, 50,
                30, 50,
                20, 50);
            ...
            switch (selection) {
                case 0 -> {
                    if (alreadyExplored) {
                        ...
                        if (battleResult) {
                            EndGameUtil.setIsEnchantedLeafCollected(Boolean.TRUE);
